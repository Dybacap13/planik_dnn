...
file_path = 'urdf/ur10e/ur10e.urdf'
...
def position_error(stats):
    def pos_error(y_true, y_pred):
        ...
        fk_true = np.array([robot.forward_kin(y)['flange'].pos for y in y_true])
        fk_pred = np.array([robot.forward_kin(y)['flange'].pos for y in y_pred])
...
def orientation_error(stats):
    def orient_error(y_true, y_pred):
        ...
        fk_true = np.array([robot.forward_kin(y)['flange'].rot for y in y_true])
        fk_pred = np.array([robot.forward_kin(y)['flange'].rot for y in y_pred])
...
class PositionError(tf.keras.callbacks.Callback):
    ...
    def eval_map(self):
        ...
        y_pred = self.model.predict(x_true, verbose=0)  # добавьте verbose=0
        ...
        x_pred = np.array([robot.forward_kin(y)['flange'].pos for y in y_pred])
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        print(f"\n📏 Position error for epoch {epoch}: {score:.6f}")
        logs['position_error'] = score
...
class OrientationError(tf.keras.callbacks.Callback):
    ...
    def eval_map(self):
        ...
        orient_pred = np.array([robot.forward_kin(y)['flange'].rot for y in y_pred])
        orient_true = np.array([robot.forward_kin(y)['flange'].rot for y in y_true])
    ...
    def on_epoch_end(self, epoch, logs=None):
        if logs is None:
            logs = {}
        score = self.eval_map()
        print(f"\n🧭 Orientation error for epoch {epoch}: {score:.6f}")
        logs['orientation-error'] = score
...
def rotmatrix_error_1(y_true, y_pred):
    global orient_true
    global orient_pred
    orient_true = np.array([np.matrix(robot.forward_kin(y)['flange'].rotation_matrix) for y in y_true])
    orient_pred = np.array([np.matrix(robot.forward_kin(y)['flange'].rotation_matrix) for y in y_pred])
