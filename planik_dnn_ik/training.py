...
from planik_dnn_mlp.simple_mlp_train import SimpleMlp
from planik_dnn_mlp.deep_mlp_train import DeepMlp
from planik_dnn_mlp.subnet_mlp_train import SixSubnetMlp
# from planik_dnn_mlp.cnn_train import Cnn
# from planik_dnn_mlp.simple_rnn_train import SimpleRnn
# from planik_dnn_mlp.lstm_train import Lstm
...
if __name__ == '__main__':
    ...
    #     dnn = SevenSubnetMlp()
    elif args.name == 'SubnetMlpPlanik':
        dnn = SixSubnetMlp()
    # elif args.name == 'Cnn':
    #     dnn = Cnn()
...
# ========== ВСТАВЬТЕ ЭТОТ КОД ЗДЕСЬ ==========
    # Отладка размеров данных и steps
    x_train_1 = np.load(f"data/{dnn.config['data_dir']}/x_train_curr1.npy")
    x_test = np.load(f"data/{dnn.config['data_dir']}/x_test.npy")

    print(f"\n=== ОТЛАДОЧНАЯ ИНФОРМАЦИЯ ===")
    print(f"Размер x_train_curr1: {x_train_1.shape}")
    print(f"Размер x_test: {x_test.shape}")
    print(f"curriculum_sizes[0]: {stats['curriculum_sizes'][0]}")
    print(f"test_size: {stats['test_size']}")
    print(f"Ожидаемый размер test: {stats['curriculum_sizes'][0] * stats['test_size']}")
    print(f"batch_size: {dnn.config['batch_size']}")

    train_steps_calculated = (stats['curriculum_sizes'][0] * (1-stats['test_size'])) // dnn.config['batch_size']
    val_steps_calculated = (stats['curriculum_sizes'][0] * stats['test_size']) // dnn.config['batch_size']

    print(f"Расчетные train_steps: {train_steps_calculated}")
    print(f"Расчетные val_steps: {val_steps_calculated}")
    print(f"Реальные train_steps (по данным): {x_train_1.shape[0] // dnn.config['batch_size']}")
    print(f"Реальные val_steps (по данным): {x_test.shape[0] // dnn.config['batch_size']}")
    print("===============================\n")
    # ========== КОНЕЦ ВСТАВКИ ==========
    # ===== ИСПРАВЛЕННЫЙ КОД =====
    # Загружаем данные для получения реальных размеров
  

    # Вычисляем steps на основе реальных данных
    train_steps = x_train_1.shape[0] // dnn.config['batch_size']
    val_steps = x_test.shape[0] // dnn.config['batch_size']
    ...
    print(f"train_steps: {train_steps} (на основе {x_train_1.shape[0]} образцов)")
    print(f"val_steps: {val_steps} (на основе {x_test.shape[0]} образцов)")
    print(f"batch_size: {dnn.config['batch_size']}")
    print("===============================\n")
    ...
    history = dnn.model.fit(
    train_generator(stats['curriculums'], dnn.config['batch_size']),
    steps_per_epoch=train_steps,  # ИСПРАВЛЕНО
    epochs=dnn.config['epochs'],
    verbose=dnn.config['verbose'],
    callbacks=callbacks_list,
    validation_data=val_generator(dnn.config['batch_size']),
    validation_steps=val_steps  # ИСПРАВЛЕНО
    )
# =============================
